...
CzDRSWallet::CzDRSWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        if (walletdb.ReadZDRSSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDRSSeed_deprecated()) {
...
bool CzDRSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDRSCount(nCountLastUsed);
    else if (!walletdb.ReadZDRSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDRSWallet::Lock()
{
    seedMaster = 0;
}

void CzDRSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDRSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDRS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDRSWallet::LoadMintPoolFromDB()
{
...
void CzDRSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDRSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDRSWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzDRSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDRS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzDRSWallet::SeedToZDRS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDRSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDRSWallet::GenerateDeterministicZDRS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDRSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDRSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
