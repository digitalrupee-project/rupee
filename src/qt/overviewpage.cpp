...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::DRS)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sDRSPercentage, QString& szDRSPercentage)
{
    int nPrecision = 2;
    ...
    szDRSPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sDRSPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelDRSPercent->setText(sPercentage);
    ui->labelzDRSPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zDRS.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in digitalrupees.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in digitalrupees.conf");
    }
    ...
    bool showDRSAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyDRSAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showDRSPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyDRSPending = watchUnconfBalance != 0;
    bool showDRSLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyDRSLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showDRSAvailable || showWatchOnlyDRSAvailable);
    ui->labelBalanceText->setVisible(showDRSAvailable || showWatchOnlyDRSAvailable);
    ui->labelWatchAvailable->setVisible(showDRSAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showDRSPending || showWatchOnlyDRSPending);
    ui->labelPendingText->setVisible(showDRSPending || showWatchOnlyDRSPending);
    ui->labelWatchPending->setVisible(showDRSPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showDRSLocked || showWatchOnlyDRSLocked);
    ui->labelLockedBalanceText->setVisible(showDRSLocked || showWatchOnlyDRSLocked);
    ui->labelWatchLocked->setVisible(showDRSLocked && showWatchOnly);
    ...
    bool showzDRSAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzDRSUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzDRSImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzDRSAvailable);
    ui->labelzBalanceMatureText->setVisible(showzDRSAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzDRSUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzDRSUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzDRSImmature);
    ui->labelzBalanceImmatureText->setVisible(showzDRSImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelDRSPercent->setVisible(showPercentages);
    ui->labelzDRSPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("DRS")
    updateDisplayUnit();
}
