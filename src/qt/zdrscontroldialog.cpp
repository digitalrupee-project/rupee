...
#include "ui_zdrscontroldialog.h"
...
std::set<std::string> ZDrsControlDialog::setSelectedMints;
std::set<CMintMeta> ZDrsControlDialog::setMints;

ZDrsControlDialog::ZDrsControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDrsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDrsControlDialog::~ZDrsControlDialog()
{
    delete ui;
}

void ZDrsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDrsControlDialog::updateList()
{
...
void ZDrsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDrsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDrs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDrsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDrsControlDialog::GetSelectedMints()
{
...
void ZDrsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
