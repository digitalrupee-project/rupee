...
#ifndef DIGITALRUPEES_ZDRSWALLET_H
#define DIGITALRUPEES_ZDRSWALLET_H
...
public:
    CzDRSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDRS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDRS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DIGITALRUPEES_ZDRSWALLET_H
