...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i digitalrupees-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i digitalrupees-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/digitalrupees-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/digitalrupees-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/digitalrupeesd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/digitalrupees.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/digitalrupees-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/digitalrupees-cli.exe $OUTDIR/bin/
